    ...
    foreach ($options['add_edit_columns'] as $column_name => $column_options) {
        ...
        switch ($column_options['type']) {
            ...
            case 'checkbox':
                ...
                //$input_rendered = '<input type="checkbox" id="' . $input_id . '" name="' . $column_name . '" value="0"' . ($values[$column_name] == 0 ? '' : 'checked') . ' />';
                $input_rendered = '<input type="checkbox" id="' . $input_id . '" name="' . $column_name . '" value="1"' . ($values[$column_name] == 1 ? 'checked="checked"' : '') . ' />';
            break;
            ...
            case 'radio':
                $input_rendered = '
                    <input type="radio" id="' . $input_id . '" name="' . $column_name . '" value="1" ' . ($values[$column_name] == '1' ? 'checked' : '') . '>One
                    <input type="radio" id="' . $input_id . '" name="' . $column_name . '" value="2" ' . ($values[$column_name] == '2' ? 'checked' : '') . '>Two
                    <input type="radio" id="' . $input_id . '" name="' . $column_name . '" value="3" ' . ($values[$column_name] == '3' ? 'checked' : '') . '>Three
                ';
                break;
        }
    ...
    foreach ($options['add_edit_columns'] as $column_name => $column_options) {
        ...
        foreach ($column_options['validate'] as $type) {
            ...
            switch ($type) {
                ...
                //Contains only 1, 2 or 3.
                case 'radio':
                    if (trim($values[$column_name]) != '') {
                        if (!is_numeric($values[$column_name])) {
                            if(($values[$column_name] != 1) || ($values[$column_name] != 2) || ($values[$column_name] != 3)) {
                                $errors[] = 'Please select 1 or 2 or 3 for ' . $column_name . '.';
                            }
                        }
                    }
                    break;
...
function data_add_edit_form_process($values, $options){
    ...
    $values['available'] = (empty($values['available'])) ? 0 : $values['available'];
...
function displaySettingsSelect($options) {
    $inputs_rendered = "";
    $currentSettings = explode(",", getSettingsFromDB());
    //print_r($currentSettings);

    $arr = getAllSticksFromDB();
    //print_r($arr);
    //print($arr[0]['title']);
    //print count($arr);
    ...
    for($i = 0; $i < count($arr); $i++) {
        for($j = 0; $j < count($currentSettings); $j++) {
            if($arr[$i]['pid'] == $currentSettings[$j]) {
                $selectedValues = '<option value="' . $arr[$i]['pid'] . '" selected>' . $arr[$i]['title'] . '</option>';
                break;
            }
            else {
                $selectedValues = '<option value="' . $arr[$i]['pid'] . '">' . $arr[$i]['title'] . '</option>';
    ...
    $inputs_rendered = '
    <p>Select Feature Sticks:
        <select name="Featured Sticks" size="' . count($arr) . '"multiple>' .
            $selectValues . '
        </select>
    </p>
    ';
    ...
    return $inputs_rendered;
}

function getSettingsFromDB() {
    global $pdo;
    $sql_queryString = "SELECT * FROM settings";
    $sql = $pdo->prepare($sql_queryString);
    //Execute the SQL query.
    $sql->execute();
    while ($row = $sql->fetch(PDO::FETCH_ASSOC)) {
        return $row['featured_sticks'];
    }
}

function getAllSticksFromDB() {
    $arr = array();
    global $pdo;
    $sql_queryString = "SELECT * FROM products";
    $sql = $pdo->prepare($sql_queryString);
    //Execute the SQL query.
    $sql->execute();
    while ($row = $sql->fetch(PDO::FETCH_ASSOC)) {
