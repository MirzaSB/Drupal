...
function admin_products_list() {
    ...
    $sql_queryString = "SELECT * FROM products ORDER BY title ASC";
        ...
        while ($row = $sql->fetch(PDO::FETCH_ASSOC)) {
            $output .= '
              <tr>
                ...
                <td>' . $row['title'] . '</td>
                <td>$' . $row['price'] . '</td>
                <td><img src="' . url('images/' . $row['image']) . '"></td>
...
function admin_products_add_edit_form($pid = '') {
    ...
    elseif ($pid != '') {
        $sql_queryString = "SELECT * FROM products WHERE pid = '" . $pid . "'";
...
function admin_products_add_edit_form_process($values) {
    ...
    //If no errors, go ahead and add the person.
        ...
        if ($clean_values['pid'] == '') {
            ...
            $sql_queryString = "INSERT INTO products (title, price, image) VALUES (" . $add_query . ")";
        }
        else {
        ...
        //Execute the appropriate SQL query. Return the boolean value, true if the row has been updated/inserted.
        $sql_insert = $pdo->prepare($sql_queryString);
        $result = $sql_insert->execute();
        notice($sql_queryString);
        ...
        if ($result) {
          ...
          notice("No row was updated or inserted for pid: " . $clean_values['pid']);
        }
    }
}
...
function admin_products_delete_product($pid) {
    global $pdo;
    $sql_deleteQuery = "DELETE FROM products WHERE pid = '" . $pid . "'";
    //Prepare the SQL statement to be executed.
    ...
    try {
        ...
        if($sql_delete->execute()) {
            ...
            notice ('The product wutg PID ' . $pid . ' was not deleted.');
        }
    }
    catch (PDOException $e) {
        print $e->getMessage();
